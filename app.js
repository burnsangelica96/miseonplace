...
function handleInvoiceFile(file) {
  ...
  reader.onload = (e) => {
    ...
    if (file.type.startsWith('image/')) {
      document.getElementById('invoice-img-preview').src = e.target.result;
  ...
  try {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        system: `You are an expert at reading supplier invoices for restaurants. Extract all line items from the invoice. Return ONLY valid JSON array — no markdown, no explanation. Each item: {"name": string, "qty": number, "unit": string, "price": number, "supplier": string}. If supplier name is visible on invoice use it, otherwise use "Unknown". For price use cost per unit (not total line cost). If you cannot determine a value use null.`,
        messages: [{
          role: 'user',
          content: [{
            type: isImage ? 'image' : 'document',
            source: { type: 'base64', media_type: mediaType, data: _invoiceB64 }
          }, {
            type: 'text',
    ...
    try {
      ...
      throw new Error('Could not parse invoice. Please check the image quality and try again.');
    }

    _invoiceItems = items.filter(i => i.name);
...
async function confirmInvoiceScan() {
  if (_invoiceWorkflow === 'update_inventory') {
    ...
    const section = 'kitchen'; // default — could prompt
    let updated = 0;
    ...
    resetInvoiceModal();
    clearInvCache();
    alert(`Done! ${updated} of ${_invoiceItems.length} items matched and updated in inventory.${updated < _invoiceItems.length ? '\n\nSome items weren\'t matched by name — add them manually or check spelling.' : ''}`);
  } else {
