...
const ROLE_PERMISSIONS = {
  owner: ['dashboard','kitchen','bar','misc','recipes','bar_program','food_cost','menu_pricing','menu_profit',
          'sop_foh','sop_boh','prep_checklist','daily_sales','order_history','financials','labor','hr',
          'team_logs','checklists','user_approvals'],
  manager: ['dashboard','kitchen','bar','misc','recipes','bar_program','food_cost','menu_pricing','menu_profit',
...
function applyRolePermissions(role) {
  ...
  // Notification bell — owner & manager only
  const bell = document.getElementById('notif-bell');
  if (bell) bell.style.display = (role === 'owner' || role === 'manager') ? 'block' : 'none';
  // Edit restaurant button — owner only
  const editBtn = document.getElementById('edit-rest-btn');
  if (editBtn) editBtn.style.display = role === 'owner' ? 'block' : 'none';
}
...
async function initApp(user) {
  ...
  // Scan for notifications in the background (owner/manager)
  if (profile.role === 'owner' || profile.role === 'manager') {
    setTimeout(() => scanForNotifications(), 1200);
  }
}
...
// ─── EDIT RESTAURANT NAME (owner only) ─────────────────────────────
...
async function saveRestaurantName() {
  ...
  await sb.from('restaurants').update({ name }).eq('id', id);

  // Update local list + the dropdown without a full reload
  const rest = restaurants.find(r => r.id === id);
  if (rest) rest.name = name;
  const sel = document.getElementById('restaurant-select');
  if (sel) sel.innerHTML = restaurants.map(r => `<option value="${r.id}"${r.id===currentRestaurantId?' selected':''}>${r.name}</option>`).join('');

  showToast();
  closeModal('edit-rest-modal');
  // Refresh dashboard title if we're on it
...
function showPage(id) {
  ...
  const loaders = {
    ...
    recipes: renderRecipes, menu_pricing: renderMenuPricing, menu_profit: renderMenuProfit,
...
async function openInvModal(section, item = null) {
  ...
  if (item) {
    ...
    document.getElementById('inv-count-method').value = item.count_method || 'number';
    toggleCountMethodFields();
    loadItemSuppliers(item.id);
  } else {
    ['inv-id','inv-name','inv-qty','inv-par','inv-cost'].forEach(id =>
      document.getElementById(id).value = '');
    document.getElementById('inv-count-method').value = 'number';
    toggleCountMethodFields();
...
// Hide the "Qty on Hand" field when the item is counted by judgment
function toggleCountMethodFields() {
  const method = document.getElementById('inv-count-method')?.value;
  const qtyField = document.getElementById('inv-qty')?.closest('.field');
  if (qtyField) {
    if (method === 'judgment') {
      qtyField.style.opacity = '0.4';
      qtyField.style.pointerEvents = 'none';
      const lbl = qtyField.querySelector('label');
      if (lbl) lbl.textContent = 'Qty on Hand (n/a for judgment)';
    } else {
      qtyField.style.opacity = '1';
      qtyField.style.pointerEvents = 'auto';
      const lbl = qtyField.querySelector('label');
      if (lbl) lbl.textContent = 'Qty on Hand';
    }
  }
}
...
async function saveInvItem() {
  ...
  const item = {
    ...
    count_method:  document.getElementById('inv-count-method')?.value || 'number',
    supplier:      '',
...
// ─── RECIPES (inventory-linked builder) ────────────────────────────

// Open a fresh "New Recipe" modal
function openRecipeModal() {
  document.getElementById('rec-id').value = '';
  document.getElementById('rec-name').value = '';
  document.getElementById('rec-cat').value = 'Main';
  document.getElementById('rec-serves').value = '1';
  ...
  document.querySelector('#modal-recipe-modal .modal-title').textContent = 'New Recipe';
  recAddIngredient(); // start with one empty row
  recCalcCost();
  openModal('recipe-modal');
}

// Add an ingredient row to the recipe builder
function recAddIngredient(name = '', qty = 1, unit = '', cost = 0, invId = '') {
  const row = document.createElement('div');
  row.className = 'rec-ing-row';
  row.style.cssText = 'display:flex;gap:8px;align-items:center;margin-bottom:6px';
  row.innerHTML = `
    <div style="position:relative;flex:2">
      <input class="rec-ing-name" placeholder="Search inventory..." value="${name}"
        oninput="recIngSearch(this)" onfocus="recIngSearch(this)"
        onblur="setTimeout(()=>recCloseDropdown(this),200)"
        data-inv-id="${invId}" data-unit="${unit}" data-cost="${cost}"
        style="width:100%;background:#FFFFFF;border:1px solid var(--border2);border-radius:2px;color:var(--text);font-family:var(--serif);font-size:13px;padding:9px 12px;outline:none">
      <div class="rec-ing-dropdown" style="display:none;position:absolute;top:100%;left:0;right:0;background:#fff;border:1px solid var(--border2);border-radius:2px;z-index:50;max-height:180px;overflow-y:auto;box-shadow:0 4px 12px rgba(25,26,30,0.12)"></div>
    </div>
    <input type="number" class="rec-ing-qty" value="${qty}" min="0" step="0.01" oninput="recCalcCost()" placeholder="Qty"
      style="flex:0 0 70px;background:#FFFFFF;border:1px solid var(--border2);border-radius:2px;color:var(--text);font-family:var(--serif);font-size:13px;padding:9px 8px;outline:none;text-align:center">
    <span class="rec-ing-unit" style="flex:0 0 50px;font-size:11px;color:var(--woodland);font-family:var(--sans)">${unit}</span>
    <span class="rec-ing-linecost" style="flex:0 0 60px;font-size:13px;color:var(--charcoal);font-family:var(--display);text-align:right">$0.00</span>
    <button type="button" onclick="this.closest('.rec-ing-row').remove();recCalcCost()" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:18px;line-height:1;flex:0 0 24px">×</button>`;
  document.getElementById('rec-ing-rows').appendChild(row);
  recCalcCost();
}

async function recIngSearch(input) {
  ...
  drop.innerHTML = matches.map(i => `<div onmousedown="recSelectIng(this)"
    data-id="${i.id}" data-name="${i.name.replace(/"/g,'&quot;')}" data-unit="${i.unit||'units'}" data-cost="${i.cost_per_unit||0}"
    style="padding:9px 12px;cursor:pointer;border-bottom:1px solid var(--border);display:flex;justify-content:space-between"
    onmouseover="this.style.background='rgba(173,158,137,0.15)'" onmouseout="this.style.background='transparent'">
    <span style="font-size:13px;color:var(--charcoal)">${i.name}</span>
    <span style="font-size:11px;color:var(--woodland)">$${(i.cost_per_unit||0).toFixed(2)}/${i.unit||'unit'}</span>
  </div>`).join('');
  drop.style.display = 'block';
}

function recSelectIng(opt) {
  const row = opt.closest('.rec-ing-row');
  const nameInput = row.querySelector('.rec-ing-name');
  nameInput.value = opt.dataset.name;
  nameInput.dataset.invId = opt.dataset.id;
  nameInput.dataset.unit = opt.dataset.unit;
  nameInput.dataset.cost = opt.dataset.cost;
  row.querySelector('.rec-ing-unit').textContent = opt.dataset.unit;
  opt.closest('.rec-ing-dropdown').style.display = 'none';
  recCalcCost();
}

function recCloseDropdown(input) {
  const drop = input.nextElementSibling;
  if (drop) drop.style.display = 'none';
}

function recCalcCost() {
  let total = 0;
  document.querySelectorAll('.rec-ing-row').forEach(row => {
    const qty = parseFloat(row.querySelector('.rec-ing-qty')?.value) || 0;
    const cost = parseFloat(row.querySelector('.rec-ing-name')?.dataset.cost) || 0;
    const lineCost = qty * cost;
    const lc = row.querySelector('.rec-ing-linecost');
    if (lc) lc.textContent = '$' + lineCost.toFixed(2);
    total += lineCost;
  });
  document.getElementById('rec-cost-display').textContent = '$' + total.toFixed(2);
  document.getElementById('rec-cost').value = total.toFixed(2);
  recCalcPct();
}
...
function recCalcPct() {
  ...
  const fcEl = document.getElementById('rec-fc-preview');
  const profitEl = document.getElementById('rec-profit-preview');
  if (cost > 0 && price > 0) {
    const pct = Math.round(cost / price * 100);
    const cls = pct <= 25 ? 'cost-good' : pct <= 32 ? 'cost-ok' : 'cost-bad';
    fcEl.innerHTML = `<span class="${cls}">${pct}%</span>`;
    if (profitEl) profitEl.textContent = '$' + (price - cost).toFixed(2);
  } else {
    fcEl.innerHTML = '—';
...
// Collect structured ingredients from the builder
function recCollectIngredients() {
  const data = [];
  document.querySelectorAll('.rec-ing-row').forEach(row => {
    const nameInput = row.querySelector('.rec-ing-name');
    ...
    if (!name) return;
    data.push({
      inv_id: nameInput.dataset.invId || null,
      name,
      qty: parseFloat(row.querySelector('.rec-ing-qty')?.value) || 0,
      unit: nameInput.dataset.unit || '',
      cost: parseFloat(nameInput.dataset.cost) || 0
    });
  });
  return data;
}
...
async function saveRecipe() {
  ...
  const ingredientsData = recCollectIngredients();
  // Human-readable text version for display
  const ingredientsText = ingredientsData.map(i =>
    `${i.qty} ${i.unit} ${i.name}`).join('\n');
  ...
  if (!rec.name) { alert('Enter a recipe name.'); return; }
  if (id) {
...
async function viewRecipe(id) {
  ...
  document.getElementById('vr-edit-btn').onclick = () => {
    ...
    document.querySelector('#modal-recipe-modal .modal-title').textContent = 'Edit Recipe';
    // Rebuild ingredient rows from saved data
    const rowsWrap = document.getElementById('rec-ing-rows');
    rowsWrap.innerHTML = '';
    const ings = r.ingredients_data || [];
    if (ings.length) {
      ings.forEach(i => recAddIngredient(i.name, i.qty, i.unit, i.cost, i.inv_id || ''));
    } else {
      recAddIngredient();
    }
    recCalcCost();
...
// ─── MENU PRICING ──────────────────────────────────────────────────
// ─── PERSONALIZED TARGET FOOD COST ─────────────────────────────────
// Calculates what food cost % THIS restaurant can afford, based on its
// real fixed costs (rent, utilities, labor) and desired profit margin.
// Returns null if there's no P&L data yet.
async function getTargetFoodCost(desiredProfitMargin = 15) {
  const { data } = await sb.from('financials').select('*')
    .eq('restaurant_id', currentRestaurantId)
    .order('created_at', { ascending: false }).limit(1);
  const pnl = data?.[0];
  if (!pnl || !pnl.revenue || pnl.revenue <= 0) return null;

  const rev = pnl.revenue;
  const laborPct = ((pnl.boh_labor || 0) + (pnl.foh_labor || 0)) / rev * 100;
  const fixedPct = ((pnl.rent || 0) + (pnl.other_expenses || 0)) / rev * 100;
  const barCogsPct = (pnl.bar_cogs || 0) / rev * 100;

  // What's left for food cost after everything else + desired profit
  // 100% = food% + bar% + labor% + fixed% + profit%
  const targetFood = 100 - barCogsPct - laborPct - fixedPct - desiredProfitMargin;

  return {
    target: Math.max(15, Math.round(targetFood)), // floor at 15% (can't go unrealistically low)
    laborPct: Math.round(laborPct),
    fixedPct: Math.round(fixedPct),
    barCogsPct: Math.round(barCogsPct),
    desiredProfit: desiredProfitMargin,
    rawTarget: targetFood,
    hasData: true
  };
}

// ─── BREAK-EVEN ────────────────────────────────────────────────────
// Roger Fields' core number: how much must this restaurant sell just
// to not lose money. Break-even = Fixed Costs ÷ Contribution Margin.
// Contribution margin = % left of each sale after VARIABLE costs (COGS
// + variable labor). Fixed costs = rent + other + the fixed portion of
// labor. We treat labor as ~50% fixed / 50% variable (industry rule of
// thumb for full-service), which is close enough to be useful.
async function getBreakEven() {
  const { data } = await sb.from('financials').select('*')
    .eq('restaurant_id', currentRestaurantId)
    .order('created_at', { ascending: false }).limit(1);
  const pnl = data?.[0];
  ...
  const rev = pnl.revenue;
  const cogs = (pnl.food_cogs || 0) + (pnl.bar_cogs || 0);
  const totalLabor = (pnl.boh_labor || 0) + (pnl.foh_labor || 0);

  // Split labor: ~50% fixed (salaried/managers/minimum staff), ~50% variable
  const fixedLabor = totalLabor * 0.5;
  const variableLabor = totalLabor * 0.5;

  // Fixed costs = rent + other expenses + fixed labor
  const fixedCosts = (pnl.rent || 0) + (pnl.other_expenses || 0) + fixedLabor;

  // Variable costs = COGS + variable labor
  const variableCosts = cogs + variableLabor;

  // Contribution margin ratio = (revenue - variable costs) / revenue
  const contributionMargin = (rev - variableCosts) / rev;
  if (contributionMargin <= 0) return { error: 'variable_too_high', fixedCosts, rev };

  // Break-even revenue = fixed costs / contribution margin ratio
  const breakEvenMonthly = fixedCosts / contributionMargin;
  const breakEvenDaily = breakEvenMonthly / 30;

  // How is the restaurant doing vs break-even?
  const pctOfBreakEven = rev / breakEvenMonthly * 100;
  const cushion = rev - breakEvenMonthly; // positive = profit zone, negative = losing
  ...
  return {
    ...
    aboveBreakEven: rev >= breakEvenMonthly,
    hasData: true
  };
}
...
async function renderMenuPricing() {
  ...
  // Compute the personalized target from real fixed costs
  const targetData = await getTargetFoodCost();
  const panelEl = document.getElementById('mp-target-panel');
  const targetInput = document.getElementById('mp-target');

  if (targetData && panelEl) {
    // Use the personalized target, and update the input to match
    if (targetInput) targetInput.value = targetData.target;
    panelEl.style.display = 'block';
    panelEl.innerHTML = `
      <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
        <div style="flex-shrink:0">
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:var(--woodland);font-family:var(--sans);font-weight:600">Your target food cost</div>
          <div style="font-size:34px;font-family:var(--display);font-weight:700;color:var(--mulled-wine);line-height:1.1">${targetData.target}%</div>
        </div>
        <div style="flex:1;min-width:220px;font-size:12.5px;color:var(--charcoal);line-height:1.6">
          Calculated from <strong>your</strong> real costs — not a generic number. With labor at ${targetData.laborPct}%, rent &amp; fixed costs at ${targetData.fixedPct}%, and a ${targetData.desiredProfit}% target profit, your dishes need to run at <strong>${targetData.target}% food cost</strong> for this restaurant to make money.
        </div>
      </div>`;
  } else if (panelEl) {
    // No P&L data — fall back to manual target with a prompt
    panelEl.style.display = 'block';
    panelEl.innerHTML = `
      <div style="font-size:12.5px;color:var(--woodland);line-height:1.6">
        💡 <strong>Want a personalized target?</strong> Enter this restaurant's monthly numbers in <span style="color:var(--mulled-wine);cursor:pointer;text-decoration:underline" onclick="showPage('financials')">P&amp;L</span> (rent, labor, utilities) and we'll calculate the exact food cost target this restaurant needs — based on its real costs, not a generic 28%.
      </div>`;
  }

  if (!recs.length) { tbody.innerHTML = ''; empty.style.display = 'block'; return; }
  empty.style.display = 'none';
  const target = parseInt(document.getElementById('mp-target')?.value || (targetData ? targetData.target : 28));
...
// ═══════════════════════════════════════════════════════════════════
// ─── MENU PROFITABILITY ────────────────────────────────────────────
// Overall view: every recipe ranked by profit, with menu health summary
// ═══════════════════════════════════════════════════════════════════

let _mpSort = 'profit';
let _mpFilter = 'all';

async function renderMenuProfit() {
  if (!currentRestaurantId) return;
  const { data } = await sb.from('recipes').select('*')
    .eq('restaurant_id', currentRestaurantId);
  let recs = (data || []).filter(r => r.plate_cost > 0 && r.menu_price > 0);

  const summaryEl = document.getElementById('mprof-summary');
  const tbody = document.getElementById('mprof-tbody');
  const empty = document.getElementById('mprof-empty');
  ...
  recs = recs.map(r => ({
    ...
    foodCostPct: r.plate_cost / r.menu_price * 100
  }));

  // Section filter (kitchen / bar / all)
  let filtered = recs;
  if (_mpFilter === 'kitchen') filtered = recs.filter(r => (r.section || 'kitchen') === 'kitchen');
  else if (_mpFilter === 'bar') filtered = recs.filter(r => r.section === 'bar');

  // Summary metrics (based on filtered set)
  const avgFc = filtered.reduce((s, r) => s + r.foodCostPct, 0) / filtered.length;
  const avgProfit = filtered.reduce((s, r) => s + r.profit, 0) / filtered.length;
  const totalItems = filtered.length;
  const bestItem = [...filtered].sort((a, b) => b.profit - a.profit)[0];
  const fcColor = avgFc <= 28 ? 'var(--green)' : avgFc <= 34 ? 'var(--amber)' : 'var(--red)';

  if (summaryEl) {
    summaryEl.innerHTML = `
      <div class="stat-card">
        <div class="stat-label">Items on Menu</div>
        <div class="stat-value">${totalItems}</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">Avg Food Cost</div>
        <div class="stat-value" style="color:${fcColor}">${avgFc.toFixed(1)}%</div>
      </div>
      <div class="stat-card">
      ...
      <div class="stat-card">
        ...
        <div class="stat-value" style="font-size:18px">${bestItem ? bestItem.name : '—'}</div>
      </div>`;
  }

  // Sort
  if (_mpSort === 'profit') filtered.sort((a, b) => b.profit - a.profit);
  else if (_mpSort === 'foodcost') filtered.sort((a, b) => a.foodCostPct - b.foodCostPct);
  else if (_mpSort === 'name') filtered.sort((a, b) => a.name.localeCompare(b.name));

  // Rank colors — top 3 green star, bottom 3 amber/red flag
  const sortedByProfit = [...filtered].sort((a, b) => b.profit - a.profit);
  const topIds = new Set(sortedByProfit.slice(0, 3).map(r => r.id));
  const bottomIds = new Set(sortedByProfit.slice(-3).map(r => r.id));

  tbody.innerHTML = filtered.map(r => {
    const fcCls = r.foodCostPct <= 25 ? 'cost-good' : r.foodCostPct <= 32 ? 'cost-ok' : 'cost-bad';
    let badge = '';
    if (topIds.has(r.id)) badge = '<span class="badge badge-green" style="font-size:9px">★ Star</span>';
    else if (bottomIds.has(r.id)) badge = '<span class="badge badge-amber" style="font-size:9px">⚑ Review</span>';
    const sectionBadge = r.section === 'bar'
      ? '<span class="badge badge-teal" style="font-size:9px">Bar</span>'
      : '<span class="badge badge-blue" style="font-size:9px">Kitchen</span>';
    return `<tr>
      <td><div style="display:flex;align-items:center;gap:8px"><span>${r.name}</span>${badge}</div></td>
      <td>${sectionBadge}</td>
      <td>$${r.plate_cost.toFixed(2)}</td>
      <td>$${r.menu_price.toFixed(2)}</td>
      <td class="${fcCls}">${r.foodCostPct.toFixed(0)}%</td>
      <td style="font-family:var(--display);font-weight:600;color:var(--green)">$${r.profit.toFixed(2)}</td>
    </tr>`;
  }).join('');
}

function mprofSetSort(sort) {
  _mpSort = sort;
  document.querySelectorAll('.mprof-sort-btn').forEach(b => {
    const active = b.dataset.sort === sort;
    b.style.background = active ? 'var(--mulled-wine)' : 'transparent';
    b.style.color = active ? 'var(--goat-milk)' : 'var(--woodland)';
    b.style.borderColor = active ? 'var(--mulled-wine)' : 'var(--border2)';
  });
  renderMenuProfit();
}

function mprofSetFilter(filter) {
  _mpFilter = filter;
  document.querySelectorAll('.mprof-filter-btn').forEach(b => {
    const active = b.dataset.filter === filter;
    b.style.background = active ? 'var(--sand)' : 'transparent';
    b.style.color = active ? 'var(--charcoal)' : 'var(--woodland)';
    b.style.borderColor = active ? 'var(--sand)' : 'var(--border2)';
...
// ─── BREAK-EVEN PANEL RENDER ───────────────────────────────────────
async function renderBreakEven() {
  const panel = document.getElementById('pnl-breakeven');
  if (!panel) return;
  const be = await getBreakEven();
  if (!be || !be.hasData) { panel.style.display = 'none'; return; }
  panel.style.display = 'block';

  if (be.error === 'variable_too_high') {
    panel.innerHTML = `<div style="font-size:13px;color:var(--red);line-height:1.6">
      ⚠ Your variable costs (food, bar, hourly labor) are eating more than 100% of each sale.
      At these cost levels there's no break-even — every sale loses money. Cut food cost or labor first.
    </div>`;
    return;
  }

  const fmt = v => '$' + Math.abs(v).toLocaleString();
  const statusColor = be.aboveBreakEven ? 'var(--green)' : 'var(--red)';
  const statusText = be.aboveBreakEven
    ? `You're <strong style="color:var(--green)">${fmt(be.cushion)} above</strong> break-even — that's your profit cushion.`
    : `You're <strong style="color:var(--red)">${fmt(be.cushion)} short</strong> of break-even — you're losing money at this sales level.`;

  // Progress toward break-even (capped at 100% for the bar, but show real %)
  const barWidth = Math.min(100, be.pctOfBreakEven);
  const barColor = be.aboveBreakEven ? 'var(--green)' : be.pctOfBreakEven >= 85 ? 'var(--amber)' : 'var(--red)';

  panel.innerHTML = `
    <div style="display:flex;align-items:baseline;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:6px">
      <div style="font-size:11px;text-transform:uppercase;letter-spacing:1.2px;color:var(--woodland);font-family:var(--sans);font-weight:700">Break-Even — your survival line</div>
      <div style="font-size:12px;color:var(--woodland)">Contribution margin: <strong>${be.contributionMargin}%</strong></div>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:14px;margin:14px 0">
      <div>
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:var(--woodland);font-family:var(--sans);font-weight:600">Need per month</div>
        <div style="font-size:26px;font-family:var(--display);font-weight:700;color:var(--mulled-wine);line-height:1.1">${fmt(be.breakEvenMonthly)}</div>
      </div>
      <div>
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:var(--woodland);font-family:var(--sans);font-weight:600">Need per day</div>
        <div style="font-size:26px;font-family:var(--display);font-weight:700;color:var(--mulled-wine);line-height:1.1">${fmt(be.breakEvenDaily)}</div>
      </div>
      <div>
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:var(--woodland);font-family:var(--sans);font-weight:600">You're selling</div>
        <div style="font-size:26px;font-family:var(--display);font-weight:700;color:${statusColor};line-height:1.1">${be.pctOfBreakEven}%</div>
      </div>
    </div>
    <div class="progress-bar" style="margin-bottom:10px"><div class="progress-fill" style="width:${barWidth}%;background:${barColor}"></div></div>
    <div style="font-size:13px;color:var(--charcoal);line-height:1.5">${statusText}</div>
    <div style="font-size:11px;color:var(--woodland);margin-top:8px;font-style:italic">Fixed costs (rent, fixed labor, overhead): ${fmt(be.fixedCosts)}/mo. You keep ${be.contributionMargin}¢ of every dollar after variable costs — that's what pays the fixed costs and then becomes profit.</div>`;
}
...
function renderInvoiceTable() {
  const resultsEl = document.getElementById('invoice-results');
  const thStyle = 'text-align:left;padding:8px 10px;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1.2px;color:var(--woodland);border-bottom:1px solid var(--border);font-family:var(--sans)';
  const tdStyle = 'padding:6px 8px;border-bottom:1px solid var(--border)';
  const inputStyle = 'background:#FFFFFF;border:1px solid var(--border2);border-radius:3px;color:var(--charcoal);font-family:var(--serif);font-size:13px;padding:7px 9px;width:100%;outline:none';

  let html = `<div style="font-size:13px;color:var(--woodland);margin-bottom:6px;font-family:var(--serif)">
    Found <strong style="color:var(--mulled-wine)">${_invoiceItems.length} item${_invoiceItems.length!==1?'s':''}</strong> — review and edit before applying. The scan isn't always perfect, so check the names, quantities and prices.
  </div>
  <div style="overflow-x:auto">
  <table style="width:100%;border-collapse:collapse;min-width:560px">
    <thead><tr>
      <th style="${thStyle};width:34%">Item</th>
      <th style="${thStyle};width:12%;text-align:center">Qty</th>
      <th style="${thStyle};width:13%">Unit</th>
      <th style="${thStyle};width:15%">$/Unit</th>
      <th style="${thStyle};width:22%">Supplier</th>
      <th style="${thStyle};width:4%"></th>
    </tr></thead><tbody>`;

  _invoiceItems.forEach((item, i) => {
    html += `<tr>
      <td style="${tdStyle}"><input value="${(item.name||'').replace(/"/g,'&quot;')}" oninput="_invoiceItems[${i}].name=this.value"
        style="${inputStyle}"></td>
      <td style="${tdStyle}"><input type="number" step="0.01" value="${item.qty||''}" oninput="_invoiceItems[${i}].qty=parseFloat(this.value)||0"
        style="${inputStyle};text-align:center"></td>
      <td style="${tdStyle}"><input value="${(item.unit||'units').replace(/"/g,'&quot;')}" oninput="_invoiceItems[${i}].unit=this.value"
        style="${inputStyle}"></td>
      <td style="${tdStyle}"><input type="number" step="0.01" value="${item.price||''}" oninput="_invoiceItems[${i}].price=parseFloat(this.value)||0"
        style="${inputStyle}"></td>
      <td style="${tdStyle}"><input value="${(item.supplier||'').replace(/"/g,'&quot;')}" oninput="_invoiceItems[${i}].supplier=this.value"
        style="${inputStyle}"></td>
      <td style="${tdStyle};text-align:center">
        <button onclick="removeInvoiceRow(${i})" title="Remove this row"
          style="background:none;border:none;color:var(--red);cursor:pointer;font-size:20px;line-height:1;padding:0 4px">×</button>
      </td>
    </tr>`;
  });

  html += `</tbody></table></div>
    <button onclick="addInvoiceRow()" class="btn btn-ghost btn-sm" style="margin-top:12px">+ Add a row</button>`;

  resultsEl.innerHTML = html;
}

function addInvoiceRow() {
  _invoiceItems.push({ name:'', qty:0, unit:'units', price:0, supplier:'' });
  renderInvoiceTable();
}

function removeInvoiceRow(i) {
  _invoiceItems.splice(i, 1);
  renderInvoiceTable();
}
...
async function saveShiftLog() {
  ...
  // Notify owner/managers of the new log
  const sectionLabel = { kitchen:'Kitchen', bar:'Bar', manager:'Manager' }[section] || section;
  await createNotification({
    type: 'team_log',
    title: `New ${sectionLabel} shift log`,
    body: `${payload.logged_by_name} submitted a close-out for ${new Date(date+'T00:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric'})}.`,
    severity: payload.incidents ? 'warning' : 'info',
    meta: { section, has_incidents: !!payload.incidents }
  });
...
// ═══════════════════════════════════════════════════════════════════
// ─── NOTIFICATION CENTER ───────────────────────────────────────────
// Central hub for everything the owner needs to know about.
// Sources: price changes, expiring certs/docs, new team logs, low stock.
// ═══════════════════════════════════════════════════════════════════

let _notifCache = [];

// Create a notification (deduped by dedupe_key)
async function createNotification({ type, title, body, severity = 'info', meta = {}, dedupeKey = null }) {
  try {
    const payload = {
      restaurant_id: currentRestaurantId,
    ...
    if (dedupeKey) {
      ...
      const { data: existing } = await sb.from('notifications')
        .select('id').eq('restaurant_id', currentRestaurantId)
        .eq('dedupe_key', dedupeKey).limit(1);
      if (existing?.length) return; // already notified
    }
    await sb.from('notifications').insert(payload);
  } catch (e) { console.warn('notif create failed', e); }
}

// Scan all sources and generate any new notifications
async function scanForNotifications() {
  if (!currentRestaurantId) return;
  const today = new Date();
  const in30 = new Date(); in30.setDate(today.getDate() + 30);

  // ── 1. Expiring certifications (health certs etc.) ──
  try {
    const { data: certs } = await sb.from('employee_certifications').select('*')
      .eq('restaurant_id', currentRestaurantId);
    for (const c of (certs || [])) {
      ...
      const days = Math.ceil((exp - today) / 86400000);
      if (days <= 30) {
        const monthKey = exp.toISOString().slice(0, 7);
        await createNotification({
          type: 'cert_expiring',
          title: `${c.cert_type || 'Certification'} expiring`,
          body: days < 0
            ? `Expired ${Math.abs(days)} days ago — request renewal from the employee.`
            : `Expires in ${days} day${days !== 1 ? 's' : ''} — request renewal soon.`,
          severity: days < 0 ? 'urgent' : days <= 14 ? 'urgent' : 'warning',
          meta: { cert_id: c.id, employee_id: c.employee_id, expiry: c.expiry_date },
          dedupeKey: `cert-${c.id}-${monthKey}`
        });
      }
    }
  } catch (e) {}

  // ── 2. Expiring employee documents ──
  try {
    const { data: docs } = await sb.from('employee_documents').select('*')
      .eq('restaurant_id', currentRestaurantId);
    for (const d of (docs || [])) {
      ...
      if (days <= 30) {
        ...
        await createNotification({
          type: 'doc_expiring',
          title: `${d.doc_name || 'Document'} expiring`,
          body: days < 0
            ? `Expired ${Math.abs(days)} days ago.`
            : `Expires in ${days} day${days !== 1 ? 's' : ''}.`,
          severity: days < 0 || days <= 14 ? 'urgent' : 'warning',
          meta: { doc_id: d.id, employee_id: d.employee_id },
          dedupeKey: `doc-${d.id}-${monthKey}`
        });
      }
    }
  } catch (e) {}

  // ── 3. Low stock (number-counted items below par) ──
  try {
    const { data: inv } = await sb.from('inventory').select('*')
      .eq('restaurant_id', currentRestaurantId);
    const low = (inv || []).filter(x =>
      (x.count_method || 'number') === 'number' &&
      x.par_level > 0 && x.qty <= x.par_level
    );
    ...
    for (const x of low) {
      await createNotification({
        type: 'low_stock',
        title: `${x.name} is low`,
        body: `${x.qty} ${x.unit || ''} on hand (par: ${x.par_level}). Time to reorder.`,
        severity: x.qty === 0 ? 'urgent' : 'warning',
        meta: { item_id: x.id, section: x.section },
        dedupeKey: `lowstock-${x.id}-${dayKey}`
      });
    }
  } catch (e) {}

  // Refresh the badge after scanning
  await refreshNotifBadge();
}

// ── Recipe recalculation when an ingredient's price changes ──
// Called after inventory cost updates. Finds recipes using that ingredient,
// recalculates their cost, and notifies if profit changed.
async function checkRecipePriceImpact(invId, itemName, oldCost, newCost) {
  if (!invId || oldCost === newCost) return;
  try {
    const { data: recipes } = await sb.from('recipes').select('*')
      .eq('restaurant_id', currentRestaurantId);
    for (const r of (recipes || [])) {
      const ings = r.ingredients_data || [];
      const affected = ings.find(i => i.inv_id === invId);
      if (!affected) continue;

      // Recalculate cost with new ingredient price
      let newPlateCost = 0;
      const updatedIngs = ings.map(i => {
        const cost = i.inv_id === invId ? newCost : i.cost;
        newPlateCost += (i.qty || 0) * cost;
        return { ...i, cost };
      });

      const oldPlateCost = r.plate_cost || 0;
      const price = r.menu_price || 0;
      if (price <= 0) continue; // no price set, skip notification

      const oldProfit = price - oldPlateCost;
      const newProfit = price - newPlateCost;
      const oldFcPct = oldPlateCost / price * 100;
      const newFcPct = newPlateCost / price * 100;

      // Update the recipe's stored cost + ingredient data
      await sb.from('recipes').update({
        plate_cost: parseFloat(newPlateCost.toFixed(2)),
        ingredients_data: updatedIngs,
        updated_at: new Date().toISOString()
      }).eq('id', r.id);

      // Only notify if the change is meaningful (> 1%)
      if (Math.abs(newFcPct - oldFcPct) >= 1) {
        const wentUp = newPlateCost > oldPlateCost;
        const suggestedPrice = newPlateCost / (oldFcPct / 100); // price to keep same FC%
        await createNotification({
          type: 'price_change',
          title: `${r.name}: cost ${wentUp ? 'up' : 'down'} (${itemName})`,
          body: `Food cost moved ${oldFcPct.toFixed(0)}% → ${newFcPct.toFixed(0)}%. Profit ${oldProfit >= newProfit ? 'drops' : 'rises'} $${Math.abs(oldProfit - newProfit).toFixed(2)}/plate.`,
          severity: wentUp && newFcPct > 35 ? 'urgent' : wentUp ? 'warning' : 'info',
          meta: {
            recipe_id: r.id, recipe_name: r.name,
            old_cost: oldPlateCost, new_cost: parseFloat(newPlateCost.toFixed(2)),
            menu_price: price, old_fc: oldFcPct, new_fc: newFcPct,
            old_profit: oldProfit, new_profit: newProfit,
            suggested_price: parseFloat(suggestedPrice.toFixed(2)),
            ingredient: itemName
          },
          dedupeKey: `pricechg-${r.id}-${newPlateCost.toFixed(2)}`
        });
      }
    }
  } catch (e) { console.warn('recipe impact check failed', e); }
}
...
async function refreshNotifBadge() {
  ...
  try {
    const { data } = await sb.from('notifications').select('id')
      .eq('restaurant_id', currentRestaurantId).eq('read', false);
...
async function renderNotifications() {
  ...
  const sevColor = { info: 'var(--blue)', warning: 'var(--amber)', urgent: 'var(--red)' };
  const typeIcon = {
    price_change: '💲', cert_expiring: '🪪', doc_expiring: '📄',
    team_log: '📋', low_stock: '📦'
  };

  listEl.innerHTML = _notifCache.map(n => {
    const time = new Date(n.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' });
    const isPriceChange = n.type === 'price_change' && !n.action_taken;
    return `<div style="display:flex;gap:12px;padding:14px;border-radius:3px;margin-bottom:8px;background:${n.read ? 'transparent' : 'rgba(173,158,137,0.12)'};border:1px solid ${n.read ? 'var(--border)' : 'rgba(86,72,59,0.18)'};border-left:3px solid ${sevColor[n.severity] || 'var(--blue)'}">
      <div style="font-size:18px;flex-shrink:0">${typeIcon[n.type] || '🔔'}</div>
      <div style="flex:1;min-width:0">
        <div style="font-size:14px;font-weight:${n.read ? '400' : '600'};color:var(--charcoal);font-family:var(--serif)">${n.title}</div>
        ${n.body ? `<div style="font-size:12px;color:var(--woodland);margin-top:3px;line-height:1.5">${n.body}</div>` : ''}
        <div style="font-size:10px;color:var(--muted);margin-top:5px;font-family:var(--sans);text-transform:uppercase;letter-spacing:0.5px">${time}</div>
        ${isPriceChange ? `<div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">
          <button class="btn btn-primary btn-sm" onclick="notifRaisePrice('${n.id}')">Raise menu price to $${(n.meta?.suggested_price || 0).toFixed(2)}</button>
          <button class="btn btn-ghost btn-sm" onclick="notifKeepPrice('${n.id}')">Keep price — show my loss</button>
        </div>` : ''}
      </div>
      <button onclick="notifDismiss('${n.id}')" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:16px;flex-shrink:0;height:fit-content">×</button>
    </div>`;
  }).join('');

  // Mark all as read after viewing (small delay)
  setTimeout(() => markAllNotifRead(), 1500);
}

async function markAllNotifRead() {
  const unread = _notifCache.filter(n => !n.read).map(n => n.id);
  if (!unread.length) return;
  await sb.from('notifications').update({ read: true }).in('id', unread);
  refreshNotifBadge();
}

// Raise the menu price as suggested
async function notifRaisePrice(notifId) {
  const n = _notifCache.find(x => x.id === notifId);
  if (!n || !n.meta?.recipe_id) return;
  const newPrice = n.meta.suggested_price;
  await sb.from('recipes').update({ menu_price: newPrice }).eq('id', n.meta.recipe_id);
  await sb.from('notifications').update({
    action_taken: 'raised_price', read: true,
    body: n.body + ` ✓ Menu price raised to $${newPrice.toFixed(2)}.`
  }).eq('id', notifId);
  showToast();
  renderNotifications();
}

// Keep the price — record the loss
async function notifKeepPrice(notifId) {
  const n = _notifCache.find(x => x.id === notifId);
  if (!n || !n.meta) return;
  const lossPerPlate = (n.meta.old_profit || 0) - (n.meta.new_profit || 0);
  await sb.from('notifications').update({
    action_taken: 'kept_price', read: true,
    body: n.body + ` — Keeping price. You absorb $${lossPerPlate.toFixed(2)} less profit per plate (food cost now ${(n.meta.new_fc || 0).toFixed(0)}%).`
  }).eq('id', notifId);
  showToast();
  renderNotifications();
}
