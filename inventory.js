    ...
    sitems.forEach(item => {
      ...
      html += `<div class="count-row" id="orec-${item.id}" style="opacity:0.45;transition:opacity .15s;flex-wrap:wrap;gap:8px;padding:10px 6px">
        ...
        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
          <input type="number" class="rec-qty" data-id="${item.id}" data-current="${item.qty}" data-name="${(item.name||'').replace(/"/g,'&quot;')}"
          ...
          <div style="display:flex;align-items:center;gap:3px">
            ...
            <input type="number" class="rec-price" placeholder="$/unit" min="0" step="0.01"
              data-oldprice="${item.cost_per_unit||0}"
...
async function submitOrderReceived() {
  ...
  const section = document.getElementById('count-section').value;
  const updates = [];
  const priceChanges = []; // track for recipe recalculation
  rows.forEach(row => {
    const cb = row.querySelector('.rec-check');
    if (!cb?.checked) return;
    const id          = cb.dataset.id;
    const name        = row.querySelector('.rec-qty')?.dataset.name || cb.dataset.name || 'item';
    ...
    const oldPrice    = parseFloat(row.querySelector('.rec-price')?.dataset.oldprice) || 0;
    ...
    if (!isNaN(newPrice) && newPrice > 0) {
      ...
      if (oldPrice > 0 && Math.abs(newPrice - oldPrice) > 0.001) {
        priceChanges.push({ id, name, oldCost: oldPrice, newCost: newPrice });
      }
    }
  ...
  // Recalculate recipes affected by price changes + notify
  if (typeof checkRecipePriceImpact === 'function') {
    for (const pc of priceChanges) {
      await checkRecipePriceImpact(pc.id, pc.name, pc.oldCost, pc.newCost);
    }
...
// The step BEFORE ordering: employees count what's on hand.
// Items counted "by number" → update on-hand qty.
// Items counted "by judgment" → tap Enough / Order Soon / Urgent.
// Anything low flows into the Order Request.
// ═══════════════════════════════════════════════════════════════════

async function openCountSheetCount(section) {
  if (!currentRestaurantId) return;
  document.getElementById('cs-section').value = section;
  const label = section === 'kitchen' ? 'Kitchen' : section === 'bar' ? 'Bar' : 'Misc';
  document.getElementById('cs-section-title').textContent = label + ' — Inventory Count';
  ...
  let html = `<div style="display:flex;gap:8px;margin-bottom:14px">
    <input id="cs-search" type="text" placeholder="Search items..."
      oninput="filterCountSheet(this.value)"
      style="flex:1;background:#FFFFFF;border:1px solid var(--border2);border-radius:2px;color:var(--text);font-family:var(--serif);font-size:13px;padding:10px 14px;outline:none">
  </div>`;

  html += `<div style="font-size:12px;color:var(--woodland);margin-bottom:16px;line-height:1.6;background:rgba(173,158,137,0.15);padding:11px 14px;border-radius:3px;border:1px solid rgba(86,72,59,0.14)">
    Walk your station and update each item. Items you count by number — type how many you have. Items by judgment — tap how you're doing. We'll flag anything low for ordering.
  </div>`;

  Object.entries(groups).forEach(([cat, catItems]) => {
    html += `<div class="count-category"><div class="count-cat-header">${cat}</div>`;
    catItems.forEach(item => {
      const method = item.count_method || 'number';
      const u = item.unit || 'units';
      if (method === 'judgment') {
        // Three-button judgment selector
        html += `<div class="count-row cs-row" data-id="${item.id}" data-name="${item.name}" data-method="judgment" data-supplier="${item.supplier||'No Supplier Assigned'}" data-unit="${u}">
          <div class="count-item-info" style="min-width:0;flex:1">
            <div class="count-item-name" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${item.name}</div>
            <div class="count-item-meta">By judgment${item.supplier ? ' · ' + item.supplier : ''}</div>
          </div>
          <div class="cs-judge" style="display:flex;gap:5px;flex-shrink:0">
            <button type="button" class="cs-jbtn" data-val="enough" onclick="setJudgment(this)"
              style="font-family:var(--sans);font-size:10px;font-weight:600;letter-spacing:0.5px;text-transform:uppercase;padding:7px 10px;border-radius:2px;border:1px solid var(--border2);background:transparent;color:var(--woodland);cursor:pointer">Enough</button>
            <button type="button" class="cs-jbtn" data-val="soon" onclick="setJudgment(this)"
              style="font-family:var(--sans);font-size:10px;font-weight:600;letter-spacing:0.5px;text-transform:uppercase;padding:7px 10px;border-radius:2px;border:1px solid var(--border2);background:transparent;color:var(--woodland);cursor:pointer">Order Soon</button>
            <button type="button" class="cs-jbtn" data-val="urgent" onclick="setJudgment(this)"
              style="font-family:var(--sans);font-size:10px;font-weight:600;letter-spacing:0.5px;text-transform:uppercase;padding:7px 10px;border-radius:2px;border:1px solid var(--border2);background:transparent;color:var(--woodland);cursor:pointer">Urgent</button>
          </div>
        </div>`;
      } else {
        // Number entry — shows current on-hand and par
        const parInfo = item.par_level > 0 ? ` · par ${item.par_level}` : '';
        html += `<div class="count-row cs-row" data-id="${item.id}" data-name="${item.name}" data-method="number" data-par="${item.par_level||0}" data-supplier="${item.supplier||'No Supplier Assigned'}" data-unit="${u}">
          <div class="count-item-info" style="min-width:0;flex:1">
            <div class="count-item-name" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${item.name}</div>
            <div class="count-item-meta">On hand: ${item.qty || 0} ${u}${parInfo}</div>
          </div>
          <div style="display:flex;align-items:center;gap:8px;flex-shrink:0">
            <input type="number" class="cs-count" value="" placeholder="${item.qty||0}" min="0" step="0.01"
              style="width:90px;text-align:center;font-size:20px;font-weight:600;padding:8px 6px;border-radius:2px;background:#FFFFFF;border:1px solid var(--border2);color:var(--charcoal);font-family:var(--display);outline:none">
            <span style="font-size:11px;color:var(--woodland);font-family:var(--sans);width:46px">${u}</span>
          </div>
        </div>`;
      }
    });
  ...
  document.getElementById('cs-date').textContent =
    new Date().toLocaleDateString('en-US', {weekday:'long', month:'long', day:'numeric', year:'numeric'});
  openModal('countsheet-modal');
}

function setJudgment(btn) {
  const group = btn.closest('.cs-judge');
  const colors = { enough: 'var(--green)', soon: 'var(--amber)', urgent: 'var(--red)' };
  group.querySelectorAll('.cs-jbtn').forEach(b => {
    b.style.background = 'transparent';
    b.style.color = 'var(--woodland)';
...
function filterCountSheet(query) {
  const q = query.toLowerCase().trim();
  ...
  document.querySelectorAll('.count-category').forEach(cat => {
    const visible = [...cat.querySelectorAll('.cs-row')].some(r => r.style.display !== 'none');
    cat.style.display = visible ? '' : 'none';
  });
}

async function saveCountSheet() {
  const section = document.getElementById('cs-section').value;
  const rows = document.querySelectorAll('.cs-row');
  const updates = [];   // { id, qty } for number items
  const lowItems = [];  // items to push to order request

  rows.forEach(row => {
    const id = row.dataset.id;
    const name = row.dataset.name;
    const method = row.dataset.method;
    const supplier = row.dataset.supplier || 'No Supplier Assigned';
    const unit = row.dataset.unit || 'units';

    if (method === 'number') {
      const input = row.querySelector('.cs-count');
      const val = input?.value;
      if (val !== '' && val != null) {
        const qty = parseFloat(val);
        if (!isNaN(qty)) {
          updates.push({ id, qty });
          const par = parseFloat(row.dataset.par) || 0;
          if (par > 0 && qty <= par) {
            lowItems.push({ name, qty: '', unit, supplier, reason: `low (${qty}/${par})` });
          }
        }
      }
    } else {
      // judgment
      const active = row.querySelector('.cs-jbtn.active');
      if (active) {
        const val = active.dataset.val;
        if (val === 'soon' || val === 'urgent') {
          lowItems.push({ name, qty: '', unit, supplier, reason: val === 'urgent' ? 'URGENT' : 'order soon' });
        }
      }
    }
  });

  if (!updates.length && !lowItems.length) {
    alert('Nothing to save yet. Count some items or mark their status first.');
    return;
  }

  // Update on-hand quantities for number items
  for (const u of updates) {
    await sb.from('inventory').update({ qty: u.qty, updated_at: new Date().toISOString() }).eq('id', u.id);
  }

  if (typeof clearInvCache === 'function') clearInvCache();
  closeModal('countsheet-modal');

  // Refresh inventory view
  if (typeof renderInv === 'function') renderInv(section);

  // If anything is low, offer to start an order request pre-filled
  if (lowItems.length) {
    const proceed = confirm(`Count saved! ${updates.length} item${updates.length!==1?'s':''} updated.\n\n${lowItems.length} item${lowItems.length!==1?'s are':' is'} running low. Start an Order Request with these pre-filled?`);
    if (proceed) {
      setTimeout(() => openCountSheetWithLowItems(section, lowItems), 200);
      return;
    }
  } else {
    showToast();
    alert(`Count saved! ${updates.length} item${updates.length!==1?'s':''} updated. Everything looks well-stocked.`);
  }
}

// Open the Order Request and pre-check the low items from the count
async function openCountSheetWithLowItems(section, lowItems) {
  await openCountSheet(section);
  // Pre-fill: check matching rows
  const lowNames = new Set(lowItems.map(i => i.name.toLowerCase()));
  document.querySelectorAll('.count-row[id^="oreq-"]').forEach(row => {
    const nameEl = row.querySelector('.count-item-name');
    if (nameEl && lowNames.has(nameEl.textContent.toLowerCase())) {
      const cb = row.querySelector('.req-check');
      if (cb && !cb.checked) { cb.checked = true; row.style.opacity = '1'; }
    }
  });
}
