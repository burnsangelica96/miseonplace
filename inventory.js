...
async function openCountSheet(section) {
  ...
  const unitOpts=['oz','lbs','kg','g','gallons','qt','cups','cases','bottles','units','ea'].map(u=>`<option value="${u}">${u}</option>`).join('');
  let html='';
  Object.entries(groups).forEach(([cat,catItems])=>{
    html+=`<div class="count-category"><div class="count-cat-header">${cat}</div>`;
    catItems.forEach(item=>{
      const safeUnit=item.unit||'units';
      const unitSelect=`<select class="count-unit-select" style="background:rgba(255,255,255,0.04);border:1px solid var(--border2);border-radius:2px;color:var(--ivory);font-family:var(--sans);font-size:12px;font-weight:300;padding:6px 8px;outline:none;cursor:pointer;width:90px">${['oz','lbs','kg','g','gallons','qt','cups','cases','bottles','units','ea'].map(u=>`<option value="${u}"${u===safeUnit?' selected':''}>${u}</option>`).join('')}</select>`;
      html+=`<div class="count-row" id="crow-${item.id}">
        <div style="display:flex;align-items:center;gap:12px;flex:1">
          <input type="checkbox" class="count-check" data-id="${item.id}"
            style="width:18px;height:18px;min-width:18px;cursor:pointer;accent-color:var(--olive);margin:0"
            onchange="toggleCountRow(this)">
          <div class="count-item-info">
            <div class="count-item-name">${item.name}</div>
            <div class="count-item-meta">Par: ${item.par_level} ${safeUnit}${item.supplier?' · '+item.supplier:''}</div>
          </div>
        </div>
        <div class="count-input-wrap" style="gap:8px">
          <input type="number" class="count-input" data-id="${item.id}" data-par="${item.par_level}"
            data-unit="${safeUnit}" data-name="${item.name}"
            data-supplier="${item.supplier||''}" data-cost="${item.cost_per_unit||0}"
            placeholder="0" min="0" step="0.01"
            oninput="highlightCountRow(this)"
            style="width:90px;text-align:center;font-size:18px;font-weight:300;padding:8px;border-radius:2px">
          ${unitSelect}
        </div>
      </div>`;
    });
...
function toggleCountRow(checkbox) {
  const row=document.getElementById('crow-'+checkbox.dataset.id);
  if (checkbox.checked) {
    row.style.opacity='1';
...
function highlightCountRow(input) {
  ...
  const cb=row.querySelector('.count-check');
  if (cb && val > 0) cb.checked=true;
  row.classList.remove('count-low','count-ok','count-watch');
  if (!par) return;
  const r=val/par;
...
async function submitCount() {
  const rows=document.querySelectorAll('.count-row[id^="crow-"]');
  const section=document.getElementById('count-section').value;
  const updates=[],orderItems=[];
  rows.forEach(row=>{
    const cb=row.querySelector('.count-check');
    const input=row.querySelector('.count-input');
    const unitSel=row.querySelector('.count-unit-select');
    if (!cb||!input) return;
    // Only save rows that are checked OR have a quantity entered
    const qty=parseFloat(input.value);
    if (!cb.checked && isNaN(qty)) return;
    const finalQty=isNaN(qty)?0:qty;
    const unit=unitSel?unitSel.value:(input.dataset.unit||'units');
    const par=parseFloat(input.dataset.par)||0;
    updates.push({id:input.dataset.id,qty:finalQty,unit,updated_at:new Date().toISOString()});
    if (par>0&&finalQty<par) orderItems.push({
      name:input.dataset.name,unit,
      supplier:input.dataset.supplier,
      cost:parseFloat(input.dataset.cost)||0,
      have:finalQty,par,need:Math.ceil(par-finalQty)
    });
  });
  if (!updates.length){alert('Please check at least one item and enter a quantity.');return;}
  const btn=document.getElementById('submit-count-btn');
  btn.textContent='Saving...'; btn.disabled=true;
  await Promise.all(updates.map(u=>sb.from('inventory').update({qty:u.qty,unit:u.unit,updated_at:u.updated_at}).eq('id',u.id)));
