...
async function submitCount() {
  ...
  rows.forEach(row => {
    const cb = row.querySelector('.req-check');
    const qty = parseFloat(row.querySelector('.req-qty')?.value);
    const unit = row.querySelector('.req-unit')?.value || 'units';
    const name = row.querySelector('.req-qty')?.dataset.name || '';
    if (cb?.checked && qty > 0) requested.push({ name, qty, unit });
  });
...
function showOrderRequestReport(items, section) {
  ...
  let html = `<div style="font-size:11px;color:var(--muted);margin-bottom:16px;letter-spacing:0.5px">${label} order request · ${date}</div>`;
  html += `<table style="width:100%;border-collapse:collapse;font-size:14px">
    <thead><tr>
      <th style="text-align:left;padding:8px 10px;font-size:9px;font-weight:500;text-transform:uppercase;letter-spacing:1.5px;color:var(--muted);border-bottom:1px solid var(--border)">Item</th>
      <th style="text-align:center;padding:8px 10px;font-size:9px;font-weight:500;text-transform:uppercase;letter-spacing:1.5px;color:var(--muted);border-bottom:1px solid var(--border)">Qty Needed</th>
      <th style="text-align:left;padding:8px 10px;font-size:9px;font-weight:500;text-transform:uppercase;letter-spacing:1.5px;color:var(--muted);border-bottom:1px solid var(--border)">Unit</th>
    </tr></thead><tbody>`;
  items.forEach(item => {
    html += `<tr>
      <td style="padding:12px 10px;border-bottom:1px solid rgba(255,255,255,0.03);font-weight:300;color:var(--ivory)">${item.name}</td>
      <td style="padding:12px 10px;border-bottom:1px solid rgba(255,255,255,0.03);text-align:center;font-size:20px;font-weight:200;color:var(--ivory)">${item.qty}</td>
      <td style="padding:12px 10px;border-bottom:1px solid rgba(255,255,255,0.03);color:var(--muted)">${item.unit}</td>
    </tr>`;
  });
  html += `</tbody></table>`;
